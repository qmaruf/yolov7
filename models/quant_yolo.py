...
class Model_quant_static(nn.Module):
    def __init__(self, cfg='yolor-csp-c.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        super(Model_quant_static, self).__init__()
    ...
    def forward(self, x, augment=False, profile=False):
        ...
        if augment:
            ...
            for si, fi in zip(s, f):
                ...
                y.append(yi)
            return self.dequant(torch.cat(y, 1)), None  # augmented inference, train
        else:
            return self.dequant(self.forward_once(x, profile))  # single-scale inference, train
...
def static_quant(_model, data_iter, deploy_device, device, save_path=None):
    ...
    if deploy_device == 'arm':
        _model.qconfig = torch.quantization.get_default_qconfig('qnnpack')
    elif deploy_device == 'x86':
        _model.qconfig = torch.quantization.get_default_qconfig('fbgemm')
    else:
        raise ValueError(f"deploy_device only support arm or x86, but got {deploy_device}")
    ...
    model_quantized = torch.quantization.quantize_dynamic(model_prepare)
    ...
    print (f'before quant size {print_size_of_model(_model)}')
    print (f'after quant size {print_size_of_model(model_quantized)}')
